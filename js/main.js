...
sending.addEventListener("click", function(){
    let kmToGo = parseInt( document.getElementById("input-km").value);
    console.log(kmToGo);

    let yourAge = parseInt( document.getElementById("input-age").value);
    console.log(yourAge);

    let normalRunning = kmToGo * kmPrice;
    console.log(normalRunning);

    if ( (isNaN(yourAge) == true) || (isNaN(kmToGo) == true)) {
        document.getElementById("ticket-price").innerHTML ="numero non trovato";
    } else {
        if (yourAge < 18){
